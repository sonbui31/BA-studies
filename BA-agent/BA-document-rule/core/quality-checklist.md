...
| 2 | **Tiếng Việt có dấu:** Nội dung tiếng Việt dùng dấu chuẩn Unicode; không có câu/đoạn kiểu "tieng Viet khong dau" ngoài tên file, ID, code/API/database token, URL, command. | ☐ |
| 3 | **Trật tự Logic:** Bố cục tuân thủ đúng template, sắp xếp mạch lạc, không lộn xộn. | ☐ |
| 4 | **Đánh số Chỉ mục — Heading:** Số thứ tự mục (§1, §1.1, §1.2, §2) tuần tự, không lặp, không nhảy cóc. | ☐ |
| 4a | **Đánh số Chỉ mục — Requirement ID:** BRQ/FR/NFR/US/TC-ID tuần tự trong cùng prefix, sub-ID tuần tự trong nhóm cha. | ☐ |
| 4b | **Đánh số Chỉ mục — Test Group/Case:** Test Group numbering (2.1→2.2→2.3) và TC-ID (TC-01-A→TC-01-B) tuần tự. | ☐ |
